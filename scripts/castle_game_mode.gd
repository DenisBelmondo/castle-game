...
var _weapon_attacks: Dictionary[StringName, Callable] = {
	&'revolver_attack': (func (weapon: ViewWeapon, _user_data: Variant) -> void:
			var pp = _players.get(weapon.owner)

			if pp is CastleGamePlayer:
				if pp.inventory.has(&'magnum_ammo'):
					pp.inventory[&'magnum_ammo'] -= 1
					pp.inventory[&'magnum_ammo'] = maxi(pp.inventory[&'magnum_ammo'], 0)
					pp.hud.current_ammo = pp.inventory[&'magnum_ammo']
					pp.hud.update()
	...
	if event is InputEventKey and event.is_pressed():
		...
		if event.keycode == KEY_1:
			...
			if 'revolver' in pp.inventory: # inventory
				...
				pp.hud.current_ammo = pp.inventory.get(&'magnum_ammo', 0)
...
func restart_game() -> void:
	change_map(R_LEVEL_1.instantiate())
	...
	if node is Character:
		...
		node.area.body_entered.connect(func (body: Node) -> void:
			...
			if is_instance_valid(p):
				...
				p.inventory[node.hint] += node.amount
				p.audio_ui.stream = preload('res://audio/sounds/pick_up_weapon.tres')
				p.audio_ui.play()
				p.hud.current_ammo = p.inventory[node.hint]
				p.hud.update()
				node.queue_free())
